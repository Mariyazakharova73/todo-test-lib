...
export const tasksReducer = (state = initialState, action: TasksAction): TasksState => {
  switch (action.type) {
    ...
    case tasksActionTypes.REMOVE_TASK:
      ...
      const filteredTasks = tasks.filter((item) => item.id !== action.taskId);
      state.tasks[action.todolistId] = filteredTasks;
      return { ...state, tasks: { ...state.tasks } };
    ...
    case tasksActionTypes.EDIT_TASK:
      ...
      return { ...state, tasks: { ...state.tasks } };
    case tasksActionTypes.ADD_EMPTY_TASKS:
...
export function addEmptyTasks(todolistId: string) {
  return {
